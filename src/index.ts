...
export default class BaiduAsr {
   ...
   * @private
   */
  private static addListener(eventName: EventName, cb: (data: RecognizerData) => void): EmitterSubscription {
    return eventEmitter.addListener(eventName, (data: RecognizerData<string | undefined>) => {
      // java传过来的是字符串
   ...
   * @param callback
   */
  static addResultListener(callback: (data: RecognizerData<RecognizerResultData | undefined>) => void): EmitterSubscription {
   ...
   * @param callback
   */
  static addErrorListener(callback: (data: RecognizerData<RecognizerResultError>) => void): EmitterSubscription {
